...
export default {
  ...
  methods: {
    ...
    _initScroll () {
      ...
      this.foodsScroll = new BScroll(wrapper2, {
        probeType: 2, // 1是一定时间 2是实时 3是实时和惯性也会触发
        click: true
      })
    ...
    _initTops () {
      ...
      // 找到所有分类里的li
      const lis = this.$refs.foodsUl.getElementsByClassName('food-list-hook') // 利用$refs快速定位
      Array.prototype.slice.call(lis).forEach(li => { // lis转换成真数组并遍历
        // 收集开始
        top += li.clientHeight
        tops.push(top)
      })
    ...
    showFood (food) {
      // 设置food
      this.food = food
      // 显示food组件 (在父组件中调用子组件对象的方法) 先标识子组件 再调用
